...
// Fuzz target for the "getchaintips" RPC (src/rpc/blockchain.cpp),
// dispatched through the real tableRPC.execute() against the shared
// FuzzNodeSetup node context (see fuzz_node_setup.h) -- exactly how a
// real RPC client's request reaches it. getchaintips itself is a static
// function in its own translation unit; dispatch-by-name through
// tableRPC is what makes calling it from here possible at all.
...
static int test_one_input_getchaintips(const uint8_t* data, size_t size)
{
    GetFuzzNodeSetup();
    ...
    request.strMethod = "getchaintips";
...
{
    return test_one_input_getchaintips(data, size);
}
...
{
    test_one_input_getchaintips(data, size);
    return 0;
}
