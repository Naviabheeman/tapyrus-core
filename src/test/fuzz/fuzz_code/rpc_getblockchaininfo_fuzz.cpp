...
// Fuzz target for the "getblockchaininfo" RPC (src/rpc/blockchain.cpp),
// dispatched through the real tableRPC.execute() -- exactly how a real
// RPC client's request reaches it -- against the shared FuzzNodeSetup
// node context (see fuzz_node_setup.h).
...
static int test_one_input_getblockchaininfo(const uint8_t* data, size_t size)
{
    GetFuzzNodeSetup();
...
{
    return test_one_input_getblockchaininfo(data, size);
}
...
{
    test_one_input_getblockchaininfo(data, size);
    return 0;
}
